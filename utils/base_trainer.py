...
class BaseTrainer(object):
    ...
    def __init__(self, options):
        ...
        self.eval_result_path = os.path.join(options.checkpoint_dir, 'eval.txt')
    ...
    def test(self):
        ...
        test_num = int(self.options.test_num_ratio * len(test_data_loader))
        out_sum = None
        for step, batch in enumerate(tqdm(test_data_loader, 
            ...
            if out_sum is None:
                ...
                for loss_name, val in out.items():
                    out_sum[loss_name] += val
        
        for loss_name, val in out_sum.items():
            out_sum[loss_name] /= test_num
        self.test_summaries(out_sum)

    def test_step(self, input_batch):
        raise NotImplementedError('You need to provide a _test_step method')

    def test_summaries(self, input_batch):
        raise NotImplementedError('You need to provide a _test_summaries method')

    def eval(self):
        raise NotImplementedError('You need to provide a _eval method')